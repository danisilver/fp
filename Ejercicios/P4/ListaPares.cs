...
public class ListaPares {
	...
	public void primero (out int i, out int j) {
		if (pri == null)
			throw new Exception ("Error primero: lista vacia");
		i = pri.x;
		j = pri.y;
	}
	...
	public void ultimo (out int i, out int j) {
		if (ult == null)
			throw new Exception ("Error ultimo: lista vacia");
		i = ult.x;
		j = ult.y;
	}
	...
	public bool esta (int i, int j) {
		...
		while (aux != null && (aux.x != i || aux.y != j))
			aux = aux.sig;
	...
	public bool eliminaElto (int x, int y) {
		// lista vacia
		if (pri == null)
			return false;// throw new Exception("Error eliminaElto: lista vacia");
			else {
			// eliminar el primero
			if (x == pri.x && y == pri.y) {
	...
	public void eliminaIni () {
		if (pri == null)
			throw new Exception ("Error eliminaIni: lista vacia");
		if (pri == ult)
			pri = ult = null;
		else
			pri = pri.sig;
	...
	public void nEsimo (int n, out int x, out int y) {
		...
		if (aux == null)
			throw new Exception ("Error nEsimo: lista con menos de " + n + " elementos");
		else {
			x = aux.x;
			y = aux.y;
		}
